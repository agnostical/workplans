...
test("update chains work_on from RULES.md through to settings.yml", () => {
  const cwd = scaffold();
  const wp = join(cwd, "workplans");
  writeFileSync(
    join(wp, "RULES.md"),
  ...
  const r = run(["update"], cwd);
  assert.equal(r.status, 0);
  const settings = readFileSync(join(wp, "settings.yml"), "utf8");
  assert.match(settings, /^projects:\n  main:\n    work_on: "https:\/\/github\.com\/acme\/backend"\n$/m);
  const readme = readFileSync(join(wp, "README.md"), "utf8");
  assert.ok(!readme.includes("work_on"), "README frontmatter cleaned");
  assert.match(readme, /# My project/, "existing README content preserved");
});

test("update migrates README constants and LOCAL.yml to the settings files", () => {
  const cwd = scaffold();
  const wp = join(cwd, "workplans");
  writeFileSync(
    join(wp, "README.md"),
  ...
  writeFileSync(join(wp, "LOCAL.yml"), `# Auto-generated by agent. Do not commit.\nwork_on: "/Users/me/repos/acme/backend"\n`);
  writeFileSync(join(cwd, ".gitignore"), "workplans/LOCAL.yml\n");

  const r = run(["update"], cwd);
  assert.equal(r.status, 0);

  const settings = readFileSync(join(wp, "settings.yml"), "utf8");
  assert.match(settings, /^tracker: "https:\/\/linear\.app\/acme\/team\/ENG"$/m);
  assert.match(settings, /^estimate_scale: "tshirt"$/m);
  assert.match(settings, /projects:\n  main:\n    work_on: "https:\/\/github\.com\/acme\/backend"/);

  const local = readFileSync(join(wp, "settings.local.yml"), "utf8");
  assert.match(local, /projects:\n  main:\n    work_on: "\/Users\/me\/repos\/acme\/backend"/);
  assert.ok(!existsSync(join(wp, "LOCAL.yml")), "LOCAL.yml removed");
  assert.match(readFileSync(join(cwd, ".gitignore"), "utf8"), /workplans\/settings\.local\.yml/);

  const readme = readFileSync(join(wp, "README.md"), "utf8");
  assert.ok(!readme.includes("---"), "empty frontmatter block removed");
  assert.match(readme, /^# My project/m, "README body preserved");

  // Idempotent: a second update leaves the migrated files alone.
  assert.equal(run(["update"], cwd).status, 0);
  assert.equal(readFileSync(join(wp, "settings.yml"), "utf8"), settings);
});
